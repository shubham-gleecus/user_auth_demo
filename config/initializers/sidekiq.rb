...
#  different db with namespace -> where 5 is the db name
...
#    $ redis-cli flushdb
# -> Delete specific redis database with (e.g. 5 as my target database):
...
# Use redis DB 5 for all job data
...
redis = { url: "redis://127.0.0.1:6379/#{redis_db}",
          # namespace: "digital-assistant-#{Rails.env}", # enable while dealing with multiple projects
          network_timeout: 5 }
# SERVER
Sidekiq.configure_server do |config|
  config.redis = redis
...
# To use delay methods on sidekiq
